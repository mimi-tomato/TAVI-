# TAVI。引継ぎ資料(2026年9月12日時点・v123)

新しいチャット(またはCodex)で作業を再開するための引継ぎ資料。旧資料(2026年7月2日時点・v100)を土台とし、フォロー・通知・ブロック・アカウント管理・DMチャットまでの実装(v101〜v122)、および旅行計画(GROUPS)のSupabase化・ルーム調整内容の永続化・関連バグ修正(v123)を反映して全面更新している。作業を始める前に、まず本資料を読み、次に作業ファイル(`C:\Dev\TAVI\index.html`)を確認すること。
...
　Codexでの作業は`C:\Dev\TAVI\index.html`を直接編集し、`git add`/`commit`/`push`でGitHub Pagesへ反映する運用に統一済み(ChatGPT/Claude.ai上での`/mnt/user-data/outputs/`経由のアップロード運用は過去の名残であり、Codexでは使わない)。**現在の最新バージョンはv123**。設計メモは `TAVI_設計方針メモ.md`(最古)、旧引継ぎ資料(v100時点)、本資料(v123時点・最新、`AGENTS.md`)の3本立てになっているため、次回以降は本資料をベースに更新していく。
...
　**次にやるべきこと(未検証)**: 上記の修正はすべてコミット・プッシュ済みだが、本番のGitHub Pages上での再検証待ち。具体的には (1) 旅グループ作成時に403エラーが出ないこと (2) 日程/予算/最終合意/スポット巡回順などを調整した状態でリロードしても内容が残ること (3) カバー画像を後から設定してもルーム名等が消えないこと (4) 友達1人だけの招待で希望集約へ進めること、の4点。検証後、次はグループチャット実装(GROUPSのSupabase化が前提だったため、v123で着手可能になった)に進む想定。
...
　プロフィール4タブの土台→My Posts→My Calendar→フォロー機能→通知システム→ブロック機能→アカウント管理→DMチャット→旅行計画(GROUPS)のSupabase化・ルーム調整内容の永続化・関連バグ修正(v123)、の順で完了した。次はv123の修正内容を本番で検証したうえで、グループチャット、またはタグ付け機能・Repostsのどちらから着手するかを、次回セッション開始時に相談して決める。

## 引継ぎ時の作業開始手順

　新しいチャット(Codex)では、本資料(`C:\Dev\TAVI\AGENTS.md`)がセッション開始時に自動的に読み込まれるので、まずこれを読んでから`C:\Dev\TAVI\index.html`を確認して作業を再開すること。会話の冒頭で運用ルール(役割統合・実装方針、および**修正後は必ずコミット&プッシュの許可を求めること**)を再確認し、ユーザーからの変更依頼に対しては、まず現状のコードを確認してから対応を検討する、という進め方を踏襲する。「旅行計画(GROUPS)のSupabase化とルーム調整フローの永続化(v123)」セクションの「次にやるべきこと(未検証)」を、まず最初に確認・消化すること。
