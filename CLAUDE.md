...
## モック機能の撤去とカレンダー初期表示月の修正(v126)

　**カレンダーが一日も選べなかった不具合**: カレンダーの初期表示月が`calYear:2026, calMonth:6`(マイカレンダー側は7月)とハードコードされていた。過去日はタップ不可の仕様のため、その月を過ぎると**開いても一日も選べない**状態になる。v125でマイカレンダーの自動反映(予定のある月へ飛ばしていた)を外したことで表面化した。`calThisYear()`/`calThisMonth()`で常に今月を初期値にし、さらに**画面を開くたびに今月へ戻す**ようにした(`resetRoomCalToThisMonth()`/`resetMyCalToThisMonth()`)。**これらは画面を開く操作(ステップ移動・ルームを開く・カレンダー画面へ遷移)からのみ呼ぶこと。** `viewDates()`のような描画関数から呼ぶと、月を移動している最中に再描画(モード切替など)が走った瞬間に今月へ戻ってしまう。
